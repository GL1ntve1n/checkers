...
class Game:
    ...
    def __animate_move(self, move: Move):
        ...
        animated_checker = self.__draw_checker(move.from_x, move.from_y, is_animated=True)
        ...
        for distance in range(abs(move.from_x - move.to_x)):
            for _ in range(100 // ANIMATION_SPEED):
                for animated in animated_checker:
                    self.__canvas.move(animated, ANIMATION_SPEED / 100 * CELL_SIZE * dx, ANIMATION_SPEED / 100 * CELL_SIZE * dy)
        ...
        for y in range(self.__field.y_size):
            for x in range(self.__field.x_size):
                ...
                if self.__field.type_at(x, y) != CheckerType.NONE and not (x == self.__animated_cell.x and y == self.__animated_cell.y):
                    self.__draw_checker(x, y)

    # Отрисовка отдельной шашки
    def __draw_checker(self, x, y, is_animated=False):
        color = 'white' if self.__field.type_at(x, y) in WHITE_CHECKERS else 'black'
        checker = self.__canvas.create_oval(
            x * CELL_SIZE + 5,
            y * CELL_SIZE + 5,
            (x + 1) * CELL_SIZE - 5,
            (y + 1) * CELL_SIZE - 5,
            fill=color,
            **({'tags': 'animated_checker'} if is_animated else {})
        )
        if self.__field.type_at(x, y) in (CheckerType.WHITE_QUEEN, CheckerType.BLACK_QUEEN):
            queen_mark = self.__canvas.create_oval(
                x * CELL_SIZE + CELL_SIZE // 2 - CELL_SIZE // 10,
                y * CELL_SIZE + CELL_SIZE // 2 - CELL_SIZE // 10,
                x * CELL_SIZE + CELL_SIZE // 2 + CELL_SIZE // 10,
                y * CELL_SIZE + CELL_SIZE // 2 + CELL_SIZE // 10,
                fill='yellow',
                **({'tags': 'animated_checker_mark'} if is_animated else {})
            )

            return [checker, queen_mark]
        return [checker]
    ...
    def __minimax_move(self, depth: int, alpha: float, beta: float, side: SideType) -> (float, list[Move]):
        # Если достигнута максимальная глубина поиска или одна из сторон не имеет более доступных ходов (или обе), происходит выход из рекурсии
        if depth == 0 or self.__field.black_score == 0 or self.__field.white_score == 0:
            try:
                # Возвращает оценку текущего состояния поля, учитывая отношение шашек одного игрока к другому
                ...
                # Если одна из сторон потеряла все шашки, возвращается положительная или отрицательная бесконечность
                return float('inf'), []

        # Если сторона, за которую играет компьютер, противоположна стороне игрока:
        if side == PLAYER_SIDE.opposite():
            # Инициализация переменных для максимального значения и хода
            max_value = -float("inf")
            max_move = []
            # Создание копии текущего состояния поля
            field_copy = Field.copy(self.__field)
            # Перебор всех доступных ходов для текущей стороны
            for move in self.__get_moves_list(side):
                # Выполнение хода и проверка наличия обязательных ходов после него
                killed = self.__handle_move(move, False) and bool(self.__get_required_moves_list(side, move))
                # Рекурсивный вызов метода для оценки хода и получения последовательности ходов для следующих итераций
                eval, seq_moves = self.__minimax_move(depth - 1 if not killed else depth, alpha, beta,
                                                      side.opposite() if not killed else side)
                # Обновление максимального значения и хода, если оценка лучше текущего максимума
                if eval > max_value:
                    max_value = eval
                    max_move = [move] + seq_moves
                # Обновление значения альфа и проверка условия для отсечения ветвления
                alpha = max(alpha, eval)
                if beta <= alpha:
                    break
                # Восстановление состояния поля перед текущим ходом
                self.__field = Field.copy(field_copy)
            # Возврат максимального значения и соответствующего хода
            return max_value, max_move
        else:
            # Аналогично, если сторона, за которую играет компьютер, совпадает со стороной игрока, но с противоположными условиями
            # Код для минимизации оценки
            # Инициализация переменных для максимального значения и хода
            min_value = float("inf")
            min_move = []
            # Создание копии текущего состояния поля
            ...
            for move in self.__get_moves_list(side):
                ...
                # Восстановление состояния поля перед текущим ходом
                self.__field = Field.copy(field_copy)
            # Возврат минимального значения и соответствующего хода
